      ...
      REM Gestione estensione eseguibile in base all'OS
      os% = FN_getOS%
      IF os% = 0 THEN
        fExe$ = base$ + ".exe"
      ELSE
      ...
      IF dir% = 2 THEN
        ...
        IF NOT noExec% THEN
          ...
          IF os% = 0 THEN
            REM Esecuzione nativa Windows
            SYS "system", "cmd.exe /c """ + fExe$ + """"
          ELSE
            REM Esecuzione Unix-like (Linux / macOS)
            SYS "system", "./""" + fExe$ + """"
          ENDIF
        ELSE
      ...
      REM ========================================================
      REM IDENTIFICAZIONE SISTEMA OPERATIVO ROBUSTA
      REM 0 = Windows, 1 = macOS, 2 = Linux
      REM ========================================================
      DEF FN_getOS%
      LOCAL os%
      ON ERROR LOCAL GOTO (win_fallback)
      os% = INKEY(-256)
      IF os% = 0 OR os% = 1 OR os% = 2 THEN = os%

      (win_fallback)
      REM Fallback infallibile per Windows (utile per versioni terminale sprovviste di SDL)
      ON ERROR LOCAL = 2
      LOCAL tmp%
      SYS "GetVersion" TO tmp%
      = 0

      REM ========================================================
      REM GENERAZIONE FILE .BBC ED ESEGUIBILE (CROSS-PLATFORM)
      REM ========================================================
      ...
      CASE os% OF
        WHEN 0:
          REM Windows: crea un file script temporaneo con CR+LF corretti per comandare bbcbasic
          F% = OPENOUT("tb_temp.txt")
          IF F% <> 0 THEN
            PROC_writeCmd(F%, "LOAD """ + fOut$ + """")
            PROC_writeCmd(F%, "SAVE """ + fBbc$ + """")
            PROC_writeCmd(F%, "QUIT")
            CLOSE#F%
          ENDIF

          REM Legge i comandi dal temp, concatena i binari e ripulisce in una sola riga di comando
          cmd$ = "cmd.exe /c ""bbcbasic.exe < tb_temp.txt >nul 2>&1 & copy /b /y bbcsdl.exe+""" + fBbc$ + """ """ + fExe$ + """ >nul & del tb_temp.txt"""

        WHEN 1:
          REM macOS: tokenizza, concatena, imposta permessi e ri-firma ad-hoc
          cmd$ = "printf 'LOAD """ + fOut$ + """\nSAVE """ + fBbc$ + """\nQUIT\n' | ./bbcbasic >/dev/null 2>&1; "
          cmd$ = cmd$ + "cat ./bbcsdl """ + fBbc$ + """ > """ + fExe$ + """ 2>/dev/null && chmod +x """ + fExe$ + """ && codesign -f -s - """ + fExe$ + """ >/dev/null 2>&1"

        OTHERWISE:
          REM Linux: printf in pipe verso ./bbcbasic, concatena e permessi base
          cmd$ = "printf 'LOAD """ + fOut$ + """\nSAVE """ + fBbc$ + """\nQUIT\n' | ./bbcbasic >/dev/null 2>&1; "
      ...
      F% = OPENIN(fBbc$)
      IF F% = 0 THEN
        PROC_printCLI("AVVISO: Impossibile generare """ + fBbc$ + """. Assicurati che gli eseguibili di BBC BASIC siano presenti nella cartella.")
      ELSE
        CLOSE#F%
      ...
      REM ========================================================
      REM SCRITTURA COMANDI PER SCRIPT WIN CON CR+LF NATIVI
      REM ========================================================
      DEF PROC_writeCmd(F%, L$)
      LOCAL i%
      IF LEN(L$) > 0 THEN
        FOR i% = 1 TO LEN(L$)
      ...
      REM ========================================================
      REM STAMPA DIRETTAMENTE SU STDOUT (CLI)
      REM ========================================================
      ...
      (prova_printf)
      REM 2. Libreria C 'printf' su Linux / macOS
      ...
      REM ========================================================
      REM PARSING RIGA DI COMANDO
      REM ========================================================
      DEF PROC_parseCLI(cmd$, RETURN in$, RETURN dict$, RETURN d%, RETURN ne%)
      LOCAL F%, a$, count%, p%
      count% = 0

      REM --- METODO 1: Lettura da /proc (Specifico Linux) ---
      ...
      (metodo_testo)
      REM --- METODO 2: Fallback su stringa @cmd$ (Windows e macOS) ---
      cmd$ = FN_trim(cmd$)
